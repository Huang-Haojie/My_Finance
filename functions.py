...
def bsm_pricer(S0, K, r, y, sigma, T, option_type):
    """calculate the fair price of option using BSM formula"""
...
def bsm_ivol(S, K, r, y, T, option_type, f, x0=None):
    """calculate the volatility of option with a given price using BSM formula"""
...
def bsm_greeks(S0, K, r, sigma, T, option_type):
    """calculate the greeks of option using BSM formula"""
...
def calculate_covariance_matrix(price_data, annualize=True):
    """calculate the covariance matrix of given price data"""
...
def calculate_portfolio_variance(weights, cov_matrix):
    """calculate the portfolio variance of given weights and covariance matrix"""
    return np.dot(weights, np.dot(cov_matrix, weights))

def calculate_portfolio_std(weights, cov_matrix):
    """calculate the portfolio standard deviation of given weights and covariance matrix"""
...
def calculate_portfolio_sharpe(weights, returns, risk_free_rate=0.02):
    """calculate the portfolio sharpe ratio (volatility/return) of given weights and returns"""
...
def optimize_minimum_variance(returns):
    """find the weights that minimize the portfolio variance"""
    cov_matrix = returns.cov()
...
def optimize_maximum_sharpe(returns, risk_free_rate=0.02):
    """find the weights that maximize the portfolio sharpe ratio (volatility/return)"""
    n_assets = len(returns.columns)
...
if __name__ == "__main__":
    S = 186.26  #current stock price
    k = 20    #exercise price of option
    r = 0.03986  #risk free rate
    y = 0.05  #dividend yield
    sigma = 0.5704  #volatility
    T = 0.3972602739726027  #time to maturity
    f=163.85  #current option price
    option_type = 'c'
